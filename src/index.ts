...
import { Slots, Teachers } from "./database";
...
app.get("/slots/:id", async (ctx) => {
  ...
  return slot ? slot : ((ctx.set.status = 404), { message: "Slot not found" });
});
...
app.get("/teachers/:id", async (ctx) => {
  ...
  return teacher ? teacher : ((ctx.set.status = 404), { message: "Teacher not found" });
});
